...
case "$arch" in
	amd64)
		...
		;;
	*)
		die "unsupported Debian architecture: $arch; current dsa releases publish linux amd64 .deb assets only"
		;;
esac
...
asset_name="$(
	awk -v pattern="dsa_.*_linux_${asset_arch}[.]deb$" '$2 ~ pattern { print $2 }' "$checksums_file"
)"
